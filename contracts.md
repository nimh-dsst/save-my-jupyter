...
**C-PANEL-01.** The Save My Jupyter side panel attaches to JupyterLab's right sidebar. It is not closable; the user opens it via the command palette (`Open Snapshot Settings`), the toolbar Snapshot button, or by clicking the sidebar tab. The header identifies the extension as `Save My Jupyter`.
...
**C-TOOLBAR-01.** Every open notebook gets a Snapshot button in its native toolbar. When the current notebook is ready, clicking the button starts the same manual snapshot flow as the panel's primary action. When snapshotting is blocked or unresolved, clicking the button reveals the side panel focused on the relevant readiness issue and does not create a snapshot.
...
**C-CMD-03.** Changing the all-cells-trigger setting from any surface produces a confirming status message: `Every executed cell will trigger snapshots.` when enabled, or `Only marked trigger cells will create automatic snapshots.` when disabled. Attempting to mark a trigger cell without any cell selected produces the warning `Select a cell before changing trigger status.`.

---

## SNAP — Manual & Trigger-Cell Snapshots

**C-SNAP-01.** Manual snapshots can be fired from three equivalent surfaces: the panel's Snapshot button, the notebook toolbar's Snapshot button, and the command palette's `Snapshot Now`. All three converge on identical behavior — same target, same artifacts, same status. Two manual clicks in quick succession always produce two snapshots; manual snapshots are never deduplicated against each other.

**C-SNAP-02.** Manual snapshots while unauthenticated are blocked with a modal dialog titled `LabArchives connection required` whose body reads `Connect LabArchives in the Save My Jupyter sidebar before creating a snapshot.` and whose single button is labeled `Open Save My Jupyter`. The panel's Readiness section additionally shows `Connect LabArchives before creating a snapshot.`.

**C-SNAP-03.** Before any snapshot fires — manual or trigger — the notebook is saved to disk via `panel.context.save()`. If save fails (read-only filesystem, permission issue), the snapshot does not proceed and the user sees the JupyterLab save error in its usual location.
...
**C-SNAP-05.** A successful snapshot's status begins with `Snapshot saved.` followed by space-separated reference clauses, each ending with a period. Possible clauses, in order: `Job <jobId>.`, `Snapshot <snapshotId>.`, `Commit <short-hash> created.` for a new commit or `Existing HEAD <short-hash> reused.` when no new commit, `Commit URL: <url>.` when a URL can be built, and `LabArchives page <name-or-id>.`. Short commit hashes are the first 12 characters when the full hash exceeds that length. Clauses without data are omitted, not rendered as "unknown." Trigger snapshots emit the same body as a JupyterLab notification.
...
**C-SNAP-07.** Trigger-cell snapshots fire automatically when a cell finishes executing successfully and either the cell's metadata marks it as a trigger or notebook-level all-cells-trigger mode is on. Cells that raise during execution never fire a snapshot. Each trigger event emits three JupyterLab notifications in sequence: a `Save My Jupyter trigger snapshot started.` info notification (auto-close 3s), a success notification with the same body as the manual success status (auto-close 5s), or an error notification with the failure message (auto-close 7s).

**C-SNAP-08.** Bursts of trigger executions on a single notebook coalesce. Trigger events arriving within roughly half a second of each other collapse into one snapshot whose `cell_ids` field is the union of every triggered cell in the burst and whose `triggering_cell_id` is the last cell that fired. Triggers spaced beyond the window remain distinct. Re-executing the same trigger cell at a later time produces a fresh snapshot, fingerprinted on the cell's execution count so the system can tell a real re-execution from duplicate event delivery.
...
**C-WATCH-01.** Watched paths are configured through the panel's Watched paths form: the user types a relative path, clicks `Add`, and sees it appear in the list with a `Remove` button. Successful registration produces the success status `Watching <normalized-path>.` and removal produces the info status `Stopped watching <path>.`. The list persists with the notebook file.

**C-WATCH-02.** Watched path entries are validated before they are persisted. Empty or whitespace-only input is rejected with `Watched paths must not be empty.`. Paths starting with `/`, `\\`, or matching a Windows drive prefix (`C:\` etc.) are rejected with `Watched paths must be relative.`. Any `..` segment is rejected with `Watched paths must stay within the notebook or repo root.`. Paths consisting only of `.` segments are rejected with `Watched paths must include at least one path segment.`. Accepted paths are stored in POSIX form (forward slashes, `.` and empty segments collapsed).

**C-WATCH-03.** Watched paths support glob patterns. Any path containing `*`, `?`, or `[` is treated as a glob; patterns like `**/*.py` match files at any depth. Non-glob paths match a literal file or directory.

**C-WATCH-04.** Watched paths are gathered at snapshot time, not polled. A snapshot includes whichever matching files exist on disk at the moment the snapshot fires. The system never watches the filesystem or fires snapshots in response to file changes; a file created and deleted between two snapshots is never captured.

**C-WATCH-05.** Watched-path resolution silently drops files that fall outside the project tree (the configured root being the git repo root when available, otherwise the notebook's parent directory). When a watched glob would otherwise pull in a file via a symlink pointing outside the project, the resolved path is checked again after `.resolve()` and dropped if it escapes. Each dropped file produces a server-side warning log so operators can audit what was excluded.

**C-WATCH-06.** Watched-path resolution silently drops files matching common credential filenames — `.env`, `.env.*`, `*.pem`, `*.key`, `id_rsa*`, `id_ed25519*`, `id_ecdsa*`, `.netrc`, `.htpasswd`, `*.p12`, `*.pfx`, `credentials`, `credentials.json` — and files under credential-bearing parent directories `.ssh/` and `.aws/`. Matching is case-insensitive on Windows and case-sensitive on POSIX. The exclusion is automatic and requires no user opt-in; the server logs a warning per skipped file.

**C-WATCH-07.** Watched-path resolution silently drops files under common build, cache, and virtual-environment directories: `.git`, `.ipynb_checkpoints`, `.mypy_cache`, `.pytest_cache`, `.ruff_cache`, `.tox`, `.uv-cache`, `.venv`, `__pycache__`, `env`, `node_modules`, `venv`. These exclusions are automatic.

**C-WATCH-08.** Watched files larger than 25 MiB each cause the snapshot to fail before any LabArchives call, with a clear "too large" error naming the offending file. Watched files that cannot be read (permission errors, etc.) raise a distinct read-failure error carrying the path.

---

## CONFIG — Configuration

**C-CONFIG-01.** The effective configuration for any snapshot is computed by merging four layers in fixed precedence, highest first: the snapshot request itself (e.g., the commit mode chosen in the panel for this snapshot), the notebook's own metadata at `notebook.metadata.save_my_jupyter`, the user's JupyterLab settings under plugin id `@save-my-jupyter/extension:plugin`, and the repo's `.save-my-jupyter.toml`. A value unset at all four layers falls back to a built-in default.

**C-CONFIG-02.** The panel's snapshot review reflects the merged policy and plan that the next snapshot will actually use. The user does not need to mentally re-run the merge to predict snapshot behavior; what the panel shows is what the snapshot does. When the resolved state cannot be computed, the panel shows the blocker or unavailable state explicitly rather than rendering blank values.
...
**C-CONFIG-04.** Repo config field defaults and shapes: `[project]` accepts `name: string` (defaults to the discovered directory name or `save-my-jupyter`) and `repo_root_strategy: "git" | "fixed"` (default `git`); other values for `repo_root_strategy` raise a validation error at parse time. `[defaults]` accepts `all_cells_trigger: bool` (false), `commit_mode: "prompt" | "always" | "never"` (`prompt`), `watch_paths: string[]` (`["**/*.py"]`), `include_notebook_file: bool` (true), `include_diff_when_dirty: bool` (true). `[labarchives]` accepts optional `target_notebook: string` and `target_root_path: string`. `[git]` accepts `stage_notebook_on_commit: bool` (true), `stage_watched_paths_on_commit: bool` (false), and `commit_message_template: string` (`snapshot: {notebook_name} {timestamp}`). A file that cannot be parsed as TOML raises a parse error carrying the file path; the snapshot falls back to higher-layer values only.
...
**C-CONFIG-07.** User preferences live in the JupyterLab settings registry under plugin id `@save-my-jupyter/extension:plugin`. Supported keys are `defaultCommitMode` (default `prompt`), `defaultRunLabel` (nullable), `defaultTags` (empty array), and `rememberCommitChoice` (default false). Older keys removed from the schema — notably `defaultExperimentContext` — are silently dropped at parse time rather than rejected, so users upgrading don't lose access to their settings.

**C-CONFIG-08.** With nothing configured anywhere, the system falls back to documented built-in defaults. Target LabArchives notebook is `Jupyter Snapshots`; in-memory fallback target path is `Notebook Log`; commit mode is `prompt`; watched paths is `["**/*.py"]`; the notebook file is included; the dirty diff is included; the notebook is staged on commit but watched paths are not; the commit message template is `snapshot: {notebook_name} {timestamp}`; the project name is `save-my-jupyter`.

**C-CONFIG-09.** Clicking **Create starter config** in the panel writes a working `.save-my-jupyter.toml` at the resolved project root, including explanatory comments and all common settings filled in. The default `target_root_path` in the generated file is `Notebook Log/{name}/{relative_notebook_path}` — no `{user_email}` placeholder, no personal data in the default path layout. On success the status reads `Created starter config at <path>.` (success). When a file already exists, the button label switches to `Ensure config exists` and clicking it shows `Config already exists at <path>.` (info) without overwriting. Without a notebook open the button is disabled and the row shows `Open a notebook before creating a repo config.` (warning). A file-write failure shows `Unable to create the starter config.` (error). A secondary hint below the row reads either `This config is already available for the current notebook.` (when loaded) or `Create a starter .save-my-jupyter.toml to share defaults for this workspace.` (with the filename rendered as code).

**C-CONFIG-10.** Both the `/state` endpoint (which feeds the panel) and the `/snapshot` endpoint (which creates a snapshot) load the notebook's `metadata.save_my_jupyter` and feed it into the four-layer merge. The user can rely on the panel's resolved view matching what the snapshot will actually do; the two code paths use the same resolution.

---

## TEMPLATE — LabArchives Path Templates

**C-TEMPLATE-01.** The LabArchives `target_root_path` may contain placeholder variables that are substituted at snapshot time. The supported set is fixed: `{name}` and `{project_name}` (project name from repo config, falling back to `save-my-jupyter`); `{user_id}` (the Jupyter user id); `{user_email}` (the authenticated LabArchives email, falling back to `unknown-email`); `{repo_name}` (the repo root's directory name, falling back to `no-repo`); `{notebook_name}` (the notebook filename); `{notebook_stem}` (the notebook filename without `.ipynb`); `{relative_notebook_path}` (the notebook's path relative to the repo root, falling back to the notebook name); `{scope_path}` (alias for `{relative_notebook_path}`); `{scope_name}` (the last segment of `{scope_path}`); `{run_label}` (the user-entered run label, falling back to `unlabeled`); `{experiment_context}` (the user-entered context, falling back to `no-context`); `{timestamp}` (ISO-8601 seconds with `:` replaced by `-`); `{date}` (`%Y-%m-%d`); `{time}` (`%H-%M-%S`); `{source}` (`manual` or `trigger_cell`); `{commit_hash}` (the commit hash, falling back to `dirty`). Removing or renaming any of these is a breaking change.
...
**C-GIT-02.** Three commit modes are supported: `prompt`, `always`, and `never`. The chosen mode comes from the standard four-layer merge: a snapshot can override it per-snapshot; notebook metadata can set a per-notebook default; user preferences can set a personal default; repo config can set a team default. When the effective mode is `prompt` at the time of execution, the user's `defaultCommitMode` is consulted, then the repo's `default_commit_mode`. If `prompt` reaches the actual snapshot execute step unresolved, the snapshot fails with an unresolved-commit-mode error rather than silently picking one.

**C-GIT-03.** Snapshot commits stage only what was asked for. With `stage_notebook_on_commit = true` (the default) the notebook file is staged; with `stage_watched_paths_on_commit = true` the configured watched paths are also staged; the `.save-my-jupyter.toml` file is also staged when modified. Unrelated working-tree changes are never staged, never committed, and never pushed. A staging failure (gitignored target, permission error) raises a distinct error with the underlying message and prevents the snapshot from proceeding.

**C-GIT-04.** The dirty diff included in a snapshot is scoped to the notebook and configured watched paths. Unrelated edits elsewhere in the working tree never appear in the snapshot's diff. Untracked watched files appear in the diff as added files. A diff-generation failure raises a distinct error and prevents the snapshot from proceeding.
...
**C-CONTENT-08.** User-entered metadata is round-tripped to the LabArchives metadata page exactly as entered: tags (comma-separated, deduplicated, whitespace-trimmed), notes (multiline), run label, experiment context, and arbitrary extra fields. A special `tagme` key in extra fields is split on `,`, `;`, and `\n`, and the resulting non-empty trimmed parts are merged into the tags list, preserving order and avoiding duplicates against existing tags.
...
**C-DEST-02.** Every snapshot directory contains a canonical page named exactly `00 Metadata` (sorting first alphabetically). It carries a table of snapshot metadata — Notebook, Notebook path, Source, Snapshot ID, Run fingerprint, Trigger cells, Commit hash, Commit status, Commit URL, Diff included (Yes/No), Extension version, Run label, Experiment context, Tags (labeled `Tags (metadata text, not native LabArchives tags)`), Notes — plus an Extra Fields table and an Artifacts index linking to the other pages in the directory.
...
**C-DEST-05.** A successful snapshot's response carries every reference the user might need to find the result: the snapshot id, the LabArchives directory name, the metadata page's id and name, and the total page count in the directory. These travel into the success status displayed in the panel (see C-SNAP-05).

**C-DEST-06.** The default LabArchives destination — when nothing has been configured anywhere — uses the project's directory name and the notebook's relative path. No user email, user id, or hostname appears in the default destination layout. Users may opt into PII-bearing placeholders explicitly by editing the template.
...
**C-QUEUE-05.** Queue state is in-memory only. A Jupyter server restart loses pending submissions, in-flight job ids, and the recent-fingerprint deduplication history. Persisted content in LabArchives is unaffected by restart.
...
**C-STATE-01.** Authentication, notebook trigger marks, watched-path lists, user preferences, and repo config all survive Jupyter server restarts — they live in the OS keyring, the notebook file, the JupyterLab settings registry, and the repo respectively. Only currently-pending queue entries and in-flight job ids are lost on restart; persisted content in LabArchives is unaffected.
...
- **OS-01.** Event-driven snapshots on watched-file changes. Watched paths are gathered at snapshot time only (C-WATCH-04).
