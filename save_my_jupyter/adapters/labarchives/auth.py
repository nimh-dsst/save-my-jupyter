"""LabArchives authentication: the real OAuth-style flow ported from the prior
implementation (labapi `Client.generate_auth_url` -> browser sign-in -> callback
-> `Client.login`), plus keyring-backed profile persistence with legacy-alias
fallback (C-AUTH-02/04/07/08). `LabArchivesAuth` is the single-user facade the
container and handlers use; it also offers a `connect_demo` shortcut for the
no-LabArchives demo mode.

GATE-UNVERIFIABLE: the labapi calls need a real LabArchives account and server
credentials (ACCESS_KEYID/ACCESS_PWD), so this path is verified by smoke test,
not the unit suite."""
...
class StoredProfile:
    ...
    @classmethod
    def from_user(
        cls, *, user_id: str, api_url: str, client: Any, user: Any
    ) -> StoredProfile:
        ...
        notebooks = cast(
            "_NotebookCollectionLike | None", getattr(user, "notebooks", None)
        )
...
class _ProfileStore:
    ...
    def load(
        ...
        return loaded_profile[1] if loaded_profile is not None else None
    ...
    def save(self, *, user_id: str, profile: StoredProfile) -> None:
        keyring_backend = self._keyring_backend
        if keyring_backend is None:
            return
        try:
            keyring_backend.set_password(
                self._service_name, user_id, profile.to_keyring_value()
            )
        except Exception:
            return
...
class AuthServiceImpl:
    ...
    def get_stored_profile(
        ...
        return self._profile_store.load(user_id=user_id, aliases=user_id_aliases)

    def _persist_profile(self, user_id: str, client: Any, user: Any) -> None:
        profile = StoredProfile.from_user(
            user_id=user_id, api_url=self._api_url, client=client, user=user
        )
        self._profile_store.save(user_id=user_id, profile=profile)
    ...
    def _get_or_restore_session(
        ...
        session = LabArchivesSession(
            user_email=stored_profile.user_email, user=user, client=client
        )
...
class LabArchivesAuth:
    """Single-user facade over AuthServiceImpl for the composition root."""
    ...
    def __init__(
        ...
        *,
        ...
        self._impl = AuthServiceImpl(keyring_backend=keyring_backend)
        self._user_id = user_id
        self._aliases = user_id_aliases
        self._demo = False

    def is_authenticated(self) -> bool:
        return self._demo or self.current_session() is not None

    def current_session(self) -> LabArchivesSession | None:
        if self._demo:
            return None
        try:
            return self._impl.get_authenticated_user(
                self._user_id, user_id_aliases=self._aliases
            )
    ...
    def user_email(self) -> str:
        ...
        return session.user_email if session is not None else ""

    def status(self) -> AuthStatusResult:
        if self._demo:
            return AuthStatusResult(status="authenticated", user_email="demo@local")
        return self._impl.get_auth_status(self._user_id, user_id_aliases=self._aliases)

    def start(self, callback_base_url: str) -> AuthStartResult:
        return self._impl.start_auth(
